...
class TestResult implements Comparable<TestResult> {
  ...
  static void logResults(Iterable<TestResult> results) {
    System.out.println(
        String.format("%1$80s | %2$8s | %3$8s | %4$9s", "Method", "Count", "Time", "#/sec"));
  ...
  TestResult(
      int parallel, int numChannels, String testName, long executionTime, long executionCount) {
