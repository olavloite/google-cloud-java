 ...
 * server. This test can be used to compare performance between different releases of the client
 * library.
 *
 * <p>The test is excluded from normal builds, but can be executed separately. The following command
 * executes this test: <code>mvn -Pstress-test -Dtest=DatabaseClientStressTest test</code>.
 */
...
public class DatabaseClientStressTest {
  ...
  @Test
  public void singleUpdateTest() throws InterruptedException, ExecutionException, TimeoutException {
    final DatabaseClient dbClient =
        spanner.getDatabaseClient(DatabaseId.of(PROJECT_ID, INSTANCE_ID, DATABASE_ID));
    Callable<Void> callable =
        new Callable<Void>() {
            ...
            Void res =
                runner.run(
                    new TransactionCallable<Void>() {
                      ...
                      public Void run(TransactionContext transaction) throws Exception {
                        assertThat(
                            transaction.executeUpdate(UPDATE_STATEMENT), is(equalTo(UPDATE_COUNT)));
                        return null;
                      }
                    });
            assertThat(runner.getCommitTimestamp(), is(notNullValue()));
  ...
  public void readWriteTransactionUpdateTest()
    ...
    runStressTest(
        callable,
        NUMBER_OF_UPDATE_TEST_EXECUTIONS / NUMBER_OF_UPDATES_IN_TX,
        NUMBER_OF_UPDATE_TEST_EXECUTIONS);
  }
  ...
  private <T> List<T> runStressTest(Callable<T> callable, int numberOfRuns, int executionCount)
    ...
    if (executor.awaitTermination(MAX_WAIT_FOR_TEST_RUN, TimeUnit.MILLISECONDS)) {
      ...
      assertThat(
          executor.getLargestPoolSize(), is(equalTo(Math.min(parallelThreads, numberOfRuns))));
    } else {
